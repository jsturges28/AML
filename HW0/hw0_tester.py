...
def build_model(n_inputs, n_hidden, n_output, activation='elu', lrate=0.001):
    ...
    model.add(Dense(n_hidden, use_bias=True, name="hidden4", activation=activation))
...
def execute_exp(args):
    ...
    ##############################
    # Run the experiment
    # Create training set: HW1 ins outs
    ...
    early_stopping_cb = keras.callbacks.EarlyStopping(patience=100, restore_best_weights=True, min_delta=0.00001)
    ...
    if not args.nogo:
        ...
        abserrors = np.abs(model.predict(ins)-outs)
...
def display_learning_curve_set(dir, base):
    ...
    plt.savefig('mse_hw0.png')
    
def display_history_set(dir, base):
    '''
    Plot the histogram of absolute error for a set of results
    ...
    for f in files:
        with open("%s/%s"%(dir,f), "rb") as fp:
            history = pickle.load(fp)
            abserrors = pickle.load(fp)
            args1 = pickle.load(fp)
            plt.hist(abserrors, 50)
            #print(history['loss'][-1])
    plt.ylabel('Count/2560')      # Plots all 2560 errors onto a histogram
    plt.xlabel('Absolute error')
    plt.legend(files, fontsize = 'small')
    plt.savefig('histogram_hw0.png')
    
def create_parser():
    '''
    Create a command line parser for the HW0 experiment
    '''
